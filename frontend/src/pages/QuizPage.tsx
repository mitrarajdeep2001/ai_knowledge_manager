...
import {
  ...
  CheckCircle, XCircle, Trophy, Brain, FileText, Upload, Hash, ChevronRight, Clock
} from 'lucide-react'
...
function GenerateModal({ onClose, onGenerated }: { onClose: () => void; onGenerated: () => void }) {
  ...
  const isValid = mode === 'topic' 
    ? form.topic.trim() 
    : (mode === 'note' ? form.noteId : form.documentId)
          ...
          {mode === 'note' && (
            <>
              <div>
                ...
                <select
                  value={form.noteId}
                  onChange={(e) => setForm((f) => ({ ...f, noteId: e.target.value }))}
                  className="input"
                >
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setForm((f) => ({ ...f, durationSeconds: parseInt(e.target.value) }))}
                className="input"
              />
            </div>
          </div>
...
function QuizRunner({ quizData, onFinish, onClose }: { quizData: QuizStartResponse; onFinish: (r: QuizSubmitResponse) => void; onClose: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const r = await quizAPI.submit({ attemptId: quizData.attemptId, answers: formattedAnswers })
      setSubmitted(true)
      onFinish(r.data)
    } catch (err: any) {
      if (timeLeft === 0) {
    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="card w-full max-w-2xl p-6 animate-fadeIn max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-lg font-semibold text-white">{quizData.title}</h2>
            <div className="flex items-center gap-3 mt-1">
              <span className="text-xs text-gray-500">Q {current + 1} / {quizData.questions.length}</span>
          ...
          <div className={clsx(
            ...
            timeLeft < 60 ? 'bg-red-900/30 border-red-700 text-red-400' : 'bg-gray-800 border-gray-700 text-gray-400'
          )}>
          ...
          {optionLabels.map((label) => (
            ...
            >
              <span className={clsx(
                ...
                answers[q.id] === label ? 'bg-brand-600 text-white' : 'bg-gray-700 text-gray-400'
              )}>
                {label}
              </span>
            ...
            {quizData.questions.map((_, i) => (
              <button
                ...
                className={clsx('w-2 h-2 rounded-full transition-all', {
                  ...
                  'bg-green-500': answers[quizData.questions[i].id] && i !== current,
                  'bg-gray-700': !answers[quizData.questions[i].id] && i !== current,
                })}
              />
            ))}
          </div>
...
function ResultsPanel({ results, onClose }: { results: QuizSubmitResponse; onClose: () => void }) {
      ...
      <div className="card w-full max-w-md p-6 animate-fadeIn">
        <div className="text-center mb-6">
          {results.isTimeExpired && (
            <div className="mb-4">
              <span className="badge bg-yellow-900/30 text-yellow-400 text-sm">⏱️ Time Expired - Auto Submitted</span>
            </div>
          )}
          <div className={clsx(
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      ...
      {results && (
        <ResultsPanel
          results={results}
          onClose={() => { setResults(null); setQuizData(null) }}
        />
      )}
      ...
      {loading ? (
        ...
        <>
          {quizSets.length > 0 && (
            <div className="mb-8">
              <h2 className="text-lg font-semibold text-white mb-4">Available Quizzes</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {quizSets.map((quiz) => (
                  <div key={quiz.id} className="card p-5 hover:border-gray-700 transition-all">
                    <div className="flex items-start justify-between mb-3">
                      <h3 className="font-semibold text-white line-clamp-2 flex-1">{quiz.title}</h3>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button onClick={() => handleStartQuiz(quiz.id)} className="btn-primary text-sm py-1.5">
          ...
          {history.length > 0 && (
            <div>
              <h2 className="text-lg font-semibold text-white mb-4">Quiz History</h2>
              <div className="space-y-3">
                {history.map((item) => (
                  <div key={item.id} className="card p-5 hover:border-gray-700 transition-all">
                    <div className="flex items-center justify-between">
                      <div>
                        <h3 className="font-semibold text-white">{item.quizTitle}</h3>
                        <div className="flex items-center gap-3 mt-1">
                          ...
                          {item.isTimeExpired && (
                            <span className="badge bg-yellow-900/30 text-yellow-400 text-xs">Time Expired</span>
                          )}
                        </div>
                      </div>
          ...
          {quizSets.length === 0 && history.length === 0 && (
            <div className="card p-16 text-center">
              ...
              <p className="text-gray-600 text-sm mb-4">Generate a quiz from your notes, documents, or a topic</p>
